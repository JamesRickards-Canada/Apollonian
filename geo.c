...
static GEN arc_midpoint(GEN c, GEN p1, GEN p2, GEN tol);
...
static GEN radialangle(GEN c, GEN p, GEN tol);
static GEN slope(GEN p1, GEN p2, GEN tol);

/*SECTION 2: INTERSECTIONS*/
static GEN arc_int(GEN c1, GEN c2, GEN tol);
static GEN arcseg_int(GEN c, GEN l, GEN tol);
...
static GEN anglediff(GEN a, GEN b, GEN tol);
static GEN divoo(GEN a, GEN b);
static int geom_check(GEN c);
static GEN shiftangle(GEN a, GEN b, GEN tol);
...
CIRCLE:
	...
	Radius > 0, but curvature can be < 0 to mean the "outside" is the inside
ARC:
	[centre, radius, curvature, start pt, end pt, start angle, end angle, dir]
	It is the arc counterclockwise from startpt to endpt, and dir=1 means oriented counterclockwise, and dir=-1 means oriented clockwise. This can also be left uninitialized if arc is not oriented.
LINE:
	[slope, intercept]
	y=slope*x+intercept unless slope=oo, where it is x=intercept instead.
SEGMENT:
	[slope, intercept, startpt, endpt, ooendptor, dir]
	dir=1 means we go from startpt to endpt in the upper half plane, and -1 means we go through infinity (only when neither endpoint is infinite). If one endpoint is oo, ooendptor stores which way we get to it. If ooendptor=1, this means the segment travels either vertically up or right, and -1 means the arc is vertically down or left.
TOLERANCE:
...
/*Returns the midpoint of the arc between p1 and p2 (counterclockwise) on c, which is either a circle or an arc. Not stack clean.*/
static GEN
arc_midpoint(GEN c, GEN p1, GEN p2, GEN tol)
{
  GEN pts = circleline_int(c, perpbis(p1, p2, tol), tol);
  GEN a1 = radialangle(c, p1, gen_0);/*No need to worry about tolerance here.*/
  GEN a2 = shiftangle(radialangle(c, p2, gen_0), a1, gen_0);
  GEN angint1 = shiftangle(radialangle(c, gel(pts, 1), gen_0), a1, gen_0);/*the angle formed by pts[1] to c with base a1.*/
  if (gcmp(a2, angint1) > 0) return gel(pts, 1);/*No need for tolerance, as if this is an issue our points p1 and p2 would be equal up to tolerance/*/
  return gel(pts, 2);
}

/*Circle with centre cent passing through p. This must be a circle, so p!=cent, and neither can be infinite. Not stack clean or gerepileupto suitable.*/
static GEN
...
{
  return line_fromsp(divoo(gen_m1, slope(p1, p2, tol)), midpoint(p1, p2));
}

/*Angle between p and the centre of c, in the range [0, 2*Pi). Not stack clean.*/
static GEN
radialangle(GEN c, GEN p, GEN tol)
{
  return shiftangle(garg(gsub(p, gel(c, 1)), lg(tol)), gen_0, tol);
}
...
slope(GEN p1, GEN p2, GEN tol)
{
  ...
  if (toleq0(fbot, tol)) fbot=gen_0;
  if (toleq0(ftop, tol)) ftop=gen_0;
  return divoo(ftop, fbot);
}


/*SECTION 2: INTERSECTIONS*/

/*Returns the intersection points of two arcs. Not stack clean or gerepileupto suitable.*/
static GEN
arc_int(GEN c1, GEN c2, GEN tol)
{
  GEN ipts = circle_int(c1, c2, tol);
  long lipts = lg(ipts);
  if (lipts == 1) return cgetg(1, t_VEC);/*Circles didn't even intersect*/
  if (lipts == 2) {/*Tangent circles.*/
    GEN pt = gel(ipts, 1);
    if (!onarc(c1, pt, tol)) return cgetg(1, t_VEC);/*Not on arc 1*/
    if (!onarc(c2, pt, tol)) return cgetg(1, t_VEC);/*Not on arc 2*/
    return ipts;/*On both arcs*/
  }/*Two intersections*/
  GEN pt1 = gel(ipts, 1), pt2 = gel(ipts, 2);
  int i1 = onarc(c1, pt1, tol);
  if (i1) i1 = onarc(c2, pt1, tol);/*Now i1==1 iff ipts[1] is on both c1 and c2*/
  int i2 = onarc(c1, pt2, tol);
  if (i2) i2 = onarc(c2, pt2, tol);/*Now i2==1 iff ipts[2] is on both c1 and c2*/
  if (i1) {
    if (i2) return ipts;
    return mkvec(pt1);
  }
  if (i2) return mkvec(pt2);
  return cgetg(1, t_VEC);
}

/*Returns the intersection points of an arc and a segment. Not stack clean or gerepileupto suitable.*/
static
GEN arcseg_int(GEN c, GEN l, GEN tol)
{
  GEN ipts = circleline_int(c, l, tol);
  long lipts = lg(ipts);
  if (lipts == 1) cgetg(1, t_VEC);/*Circle and line did not intersect*/
  if (lipts == 2) {/*Tangent line*/
    GEN pt = gel(ipts, 1);
    if (!onarc(c, pt, tol)) return cgetg(1, t_VEC);/*Not on arc*/
    if (!onseg(l, pt, tol)) return cgetg(1, t_VEC);/*Not on segment*/
    return ipts;/*On both*/
  }/*Two intersections*/
  GEN pt1 = gel(ipts, 1), pt2 = gel(ipts, 2);
  int i1 = onarc(c, pt1, tol);
  if (i1) i1 = onseg(l, pt1, tol);/*Now i1==1 iff ipts[1] is on both c and l*/
  int i2 = onarc(c, pt2, tol);
  if (i2) i2 = onseg(l, pt2, tol);/*Now i2==1 iff the ipts[2] is on both c and l*/
  if (i1) {
    if (i2) return ipts;
    return mkvec(pt1);
  }
...
//Rename this mobius_point or something
...
/*Returns -1 if x<y, 0 if x==y, 1 if x>y (x, y are t_REAL/t_INT/t_FRAC/t_INFINITY). Accounts for the tolerance, so will deem x==y if they are equal up to tol AND at least one is inexact. Infinity is always taken to be +oo.*/
static int
...
/*Returns the angle a - b (up to tolerance) in the range [0, 2*Pi)*/
static GEN
anglediff(GEN a, GEN b, GEN tol)
{
  pari_sp av = avma;
  GEN twopi = Pi2n(1, lg(tol));
  GEN diff = gmod(gsub(a, b), twopi);
  if (toleq(diff, twopi, tol) || toleq0(diff, tol)) return gc_const(av, gen_0);
  return gerepileuptoleaf(top, diff);
}

/*Divides a and b, and allows for oo and division by 0. Returns oo for x/0 and oo/x (any x, even 0 or oo), 0 for x/oo with x!=oo, and treats all infinities as +oo.*/
static GEN
divoo(GEN a, GEN b)
{
  if (gequal0(b)) return mkoo();
...
/*Shifts the given angle a by multiples of 2*Pi into the range [b, b+2*Pi) (up to tolerance).*/
static GEN
shiftangle(GEN a, GEN b, GEN tol)
{
  pari_sp av = avma;
  GEN diff = anglediff(a, b, tol);
  if (gequal0(diff)) return gerepilecopy(av, b);
  return gerepileuptoleaf(top, gadd(b, diff));
}
