...
/*Finds equations for all circles in the ACP generated by v that are at most the bound B. v can be integral or not. Can also pass in a maximal depth.*/
GEN
apol_circles(GEN v, GEN B, long depth, GEN maxxval, long prec)
{
  pari_sp av = avma;
  long t = typ(B);
  if (t != t_INT && t != t_FRAC && t != t_REAL) pari_err_TYPE("B must be an integer, rational number, or real number.", B);
  int isint;
  GEN w = apol_fix(v, &isint, prec);
  if (!w) pari_err_TYPE("v is not a Descartes quadruple.", v);
  void (*move1)(GEN, int);
  ...
  if (!maxxval) return gerepilecopy(av, vec_shorten(vfound, foundind));
